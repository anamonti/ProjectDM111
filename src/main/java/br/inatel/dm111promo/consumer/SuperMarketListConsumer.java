...
public class SuperMarketListConsumer {
    ...
    public SuperMarketListConsumer(ObjectMapper objectMapper, ProjectSubscriptionName subscriptionName, PromoByUserRepository promoByUserRepository, PromoRepository promoRepository) {
...
//        }
    ...
    private void checkAndAddPromo(SuperMarketListMessage data) {
        List<Promo> promoList = null;

        try {
            promoList = promoRepository.findPromosBySupermarketList(data.products());
    ...
    private PromoByUser buildPromoByUser(List<String> productsFromList, List<Promo> promos) {
        ...
        for (Promo promo : promos) {
            ...
            promoByUser.setName(promo.getName());
            promoByUser.setStarting(promo.getStarting());
            promoByUser.setExpiration(promo.getExpiration());
            promoByUser.setProductsForYou(products);
